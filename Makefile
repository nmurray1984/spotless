...
train:
	...
	docker build -t train-image . && \
	docker run -u $(USER):$(GROUP) -it -v $(PWD):/tmp/code -v ~/training:/tmp/training:rw --env-file .env train-image python3 /tmp/code/scripts/train.py 2>&1 | tee -a /tmp/training/run-history/stdouterr.txt && \
